...
grpc::Status CraneCtldForSignServiceImpl::SignUserCertificate(
  ...
  if (!g_config.ListenConf.AllowedNodes.empty()) {
    ...
    if (!resolve_result ||
        !g_config.ListenConf.AllowedNodes.contains(name_list[0])) {
  ...
  if (!result) {
    ...
    response->set_ca_certificate(
        g_config.ListenConf.ExternalCerts.CaContent);
  }

  return grpc::Status::OK;
}

void CtldForSignServer::Shutdown() {
  m_server_->Shutdown(std::chrono::system_clock::now() +
                      std::chrono::seconds(1));
}

CtldForSignServer::CtldForSignServer(
    const Config::CraneCtldListenConf &listen_conf) {
  m_service_impl_ = std::make_unique<CraneCtldForSignServiceImpl>(this);
  ...
  if (!m_server_) {
    CRANE_ERROR("Cannot start CraneCtld For Sign server!");
    std::exit(1);
  }
  CRANE_INFO("CraneCtld For Sign Server is listening on {}:{}",
