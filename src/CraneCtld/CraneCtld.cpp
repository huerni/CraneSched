    ...
    try {
      ...
      if (config["UseTls"] && config["UseTls"].as<bool>()) {
        ...
        if (config["InternalCertFilePath"]) {
          ...
          try {
            internal_certs.CertContent = util::ReadFileIntoString(
                internal_certs.CertFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read internal cert file error: {}", e.what());
            std::exit(1);
          }
          if (internal_certs.CertContent.empty()) {
            CRANE_ERROR(
        ...
        if (config["InternalKeyFilePath"]) {
          internal_certs.KeyFilePath = config["InternalKeyFilePath"].as<std::string>();

          try {
            internal_certs.KeyContent = util::ReadFileIntoString(
                internal_certs.KeyFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read internal key file error: {}", e.what());
            std::exit(1);
          }
          if (internal_certs.KeyContent.empty()) {
            CRANE_ERROR(
        ...
        if (config["InternalCaFilePath"]) {
          internal_certs.CaFilePath = config["InternalCaFilePath"].as<std::string>();

          try {
            internal_certs.CaContent = util::ReadFileIntoString(
                internal_certs.CaFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read internal ca file error: {}", e.what());
            std::exit(1);
          }
          if (internal_certs.CaContent.empty()) {
            CRANE_ERROR(
        ...
        if (config["ExternalCertFilePath"]) {
          external_certs.CertFilePath = config["ExternalCertFilePath"].as<std::string>();

          try {
            external_certs.CertContent = util::ReadFileIntoString(
                external_certs.CertFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read external cert file error: {}", e.what());
            std::exit(1);
          }
          if (external_certs.CertContent.empty()) {
            CRANE_ERROR(
        ...
        if (config["ExternalKeyFilePath"]) {
          external_certs.KeyFilePath = config["ExternalKeyFilePath"].as<std::string>();

          try {
            external_certs.KeyContent = util::ReadFileIntoString(
                external_certs.KeyFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read external key file error: {}", e.what());
            std::exit(1);
          }
          if (external_certs.KeyContent.empty()) {
            CRANE_ERROR(
        ...
        if (config["ExternalCaFilePath"]) {
          external_certs.CaFilePath = config["ExternalCaFilePath"].as<std::string>();

          try {
            external_certs.CaContent = util::ReadFileIntoString(
                external_certs.CaFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read external ca file error: {}", e.what());
            std::exit(1);
          }
          if (external_certs.CaContent.empty()) {
            CRANE_ERROR(
