...
SocketPortPair GetX11Socket() {
  ...
  uint32_t min = 6020, max = 6099;
  uint32_t num = max - min + 1;

  srandom(getpid());
  uint32_t port = min + (random() % num);

  for (uint32_t i = 0; i < num; i++) {
    if (fd < 0) {
      const int one = 1;
      if ((fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
        CRANE_ERROR("failed to create x11 socket.");
        return {-1, -1};
      }
      if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(int)) < 0) {
        CRANE_ERROR("failed to setsockopt.");
        close(fd);
        return {-1, -1};
      }
    }
    ...
    if (bind(fd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) continue;
    if (!listen(fd, 4096)) return {fd, port};

    CRANE_ERROR("failed to listen");

    if (close(fd)) CRANE_ERROR("failed to close");
    fd = -1;
  ...
  CRANE_ERROR(
      " all ports in range (%u, %u) exhausted, cannot establish listening port",
      min, max);
